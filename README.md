...
dscli.el 为 [dscli](https://github.com/nanjunjie/dscli) 命令行工具提供 Emacs 界面，让你在 Emacs 中无缝使用 DeepSeek 编程助手。
...
**注意**：使用带前缀的 `dscli-chat` 要求当前缓冲区关联文件（非临时缓冲区）。

### 等待动画

在等待 DeepSeek 响应时，输出缓冲区会显示 ⏳ 旋转动画。dscli 输出中的特殊标记会被处理：

- `<!-- DS-CLI-WAITING-START -->`：开始等待动画
- `<!-- DS-CLI-WAITING-PROGRESS:N -->`：更新进度到 N
...
动画间隔通过 `dscli-animation-interval` 配置（默认 0.3 秒，最小 0.1 秒）。

### 自动保存

输出缓冲区内容会自动保存到文件系统，防止数据丢失：
...
| 模块 | 文件 | 职责 |
|------|------|------|
| 主模块 | `dscli-main.el` | 主入口点和用户界面 |
| 配置 | `dscli-config.el` | 所有可自定义变量和配置 |
| 项目 | `dscli-project.el` | 项目识别和隔离 |
| 进程 | `dscli-process.el` | 进程创建、管理和终止 |
| 界面 | `dscli-ui.el` | 缓冲区显示和窗口管理 |
| 动画 | `dscli-animation.el` | 等待动画处理 |
| 保存 | `dscli-save.el` | 输出自动保存 |
| 上下文 | `dscli-context.el` | 编辑上下文感知 |
| 汇总 | `dscli-all.el` | 一键加载所有模块（开发用） |
