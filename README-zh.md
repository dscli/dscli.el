...
dscli.el 为 [dscli](https://github.com/dscli/dscli) 命令行工具提供
...
dscli.el 为 **Emacs daemon + emacsclient** 架构而设计。
运行 `M-x dscli-chat` 时，dscli.el 自动为 dscli 进程设置以下环境变量：
...
> ⚠️ **注意**：`C-c C-c` 在输入和输出缓冲区中含义不同——输入缓冲区中为
> 「发送」，输出缓冲区中为「中断进程」。请确认当前活跃缓冲区，避免误操
> 作。
